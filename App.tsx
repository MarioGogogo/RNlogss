...
function App() {
  ...
  // Phase 2 JSI helper — 补全 Go 服务端要求的字段，避免 jsonb 列收到 null
  ...
  // Phase 2 Actions (复刻 Phase 1 按钮，使用 JSI 写入)
  const phase2Actions: LogItem[] = [
    {
      id: 101,
      label: '打手动日志',
      color: '#2563eb',
    ...
    },
    {
      ...
      onPress: () => {
        jsiWrite([{
          id: genId(), level: 1, message: `JSI 用户操作 #${logCount + 1}`, timestamp: Date.now(),
          tag: 'jsi_breadcrumb', data: {index: logCount + 1, category: 'user_action'},
        }]);
      },
    },
    {
      ...
      <ScrollView contentContainerStyle={styles.container}>
        {activeTab === 'phase1' ? (
          ...
          <View>
            ...
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>手动日志</Text>
              {renderButtons([phase2Actions[0]])}
            </View>

            {/* JS 常见错误 */}
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>JS 常见错误 (JSI 记录)</Text>
              {renderButtons(phase2Actions.slice(1, 7))}
            </View>

            {/* 网络与异步 */}
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>网络与异步 (JSI 记录)</Text>
              {renderButtons(phase2Actions.slice(7, 10))}
            </View>

            {/* 操作与面包屑 */}
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>操作与面包屑 (JSI 记录)</Text>
              {renderButtons(phase2Actions.slice(10, 12))}
            </View>

            {/* JSI 工具 */}
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>JSI 工具</Text>
              {renderButtons(phase2Actions.slice(12))}
            </View>
